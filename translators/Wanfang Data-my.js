{
	"translatorID": "5577a9ca-ed2e-42cc-b86f-c14fecb05607",
	"label": "Wanfang Data-my",
	"creator": "will",
	...
	"lastUpdated": "2022-04-08 07:25:52"
}
...
function detailScrape(doc,url)
{
	var newItem = new Zotero.Item("journalArticle");
	title = ZU.xpath(doc,'//div[@class = "detailTitelInner"]/span[@class="detailTitleCN"]');
	newItem.title = title[0].innerText;
	// Z.debug(newItem.title);
	abstractNote = ZU.xpath(doc,'//div[@class = "detailList"]/div[@class="summary"]')
	newItem.abstractNote = abstractNote[0].innerText;
	// Z.debug(newItem.abstractNote);
	doi = ZU.xpath(doc,'//div[@class="doi list"]//div[@class = "itemUrl"]/a');
	newItem.DOI = doi[0].innerText;
	// Z.debug(newItem.DOI);
	authors = ZU.xpath(doc,'//div[@class="author list"]//div[@class = "itemUrl"]/a');
	// Z.debug(authors);
	newItem.language = "ZH-CN"
	publicationTitle = ZU.xpath(doc,'//div[@class="serialTitle list"]//div[@class = "itemUrl"]/a');
	newItem.publicationTitle = publicationTitle[0].innerText;
	// Z.debug(newItem.publicationTitle);
	volumn_issue= ZU.xpath(doc,'//div[@class="getYear list"]//div[@class = "itemUrl"]/a')[0].innerText;
	// Z.debug(volumn_issue);
	volumn = volumn_issue.split("(")[0].split(", ")[1];
	newItem.volume = volumn;
	issue = volumn_issue.split("(")[1].split(")")[0];
	newItem.issue = issue;
	date = ZU.xpath(doc,'//div[@class="getYear list"]//div[@class = "itemUrl"]/a')[0].innerText.split(",")[0];
	// Z.debug(date);
	if(date)
	{
	newItem.date = date;
	...
	// pdflink = ZU.xpath(doc,'//div[@class="detailIcon"]//a[@class="download"]/@href')[0].innerText;
	// if(pdflink)
	// {
	// newItem.attachments = [{
	// 			title: "Full Text PDF",
	...
	{
		// Z.debug(id);
		author = authors[id].innerText.trim();
		// Z.debug(author.substring(0,1));
		// Z.debug(author.substring(1));
		// author = authors.innerText[id]["text"].trim();
		// break;
		newItem.creators.push({lastName:author.slice(0,1),firstName:author.slice(1),creatorType:'author'});
	}
	...
	// Z.debug(newItem.publicationTitle);
	// Z.debug(newItem.volume);
	// Z.debug(newItem.issue);
